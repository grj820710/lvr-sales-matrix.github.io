...
**改檔名等於改網址。** 舊網址不會自動保留轉址。改名後舊頁面會被當成
「沒有對應來源檔」而在下次執行時刪除，等同搬家。

## 下架建案

把 `sources/` 底下的 xls 刪掉，push 上去即可。下次執行時會一併移除
`docs/` 與 `data/` 中對應的檔案，首頁清單也會跟著更新。

清理有一道保護：**只要有任何一個來源檔處理失敗，就會整個略過清理步驟**。
否則一次解析失敗（檔案毀損、格式不符）就會把還在用的頁面一起刪掉。
這種情況日誌會印出「有來源處理失敗，略過清理步驟」，修好來源檔再跑一次即可。

## 欄位怎麼決定

欄位取自「棟及號」，各建案的寫法不同，目前支援三種：

| 棟及號 | 欄位 | 表頭顯示 | 樓層來源 |
| --- | --- | --- | --- |
| `B棟05F-05號` | `B05` | B棟05號 | 棟及號本身 |
| `A棟1號` | `A01` | A棟1號 | 樓別/樓高 |
| `B7棟0號` | `B7` | B7棟 | 樓別/樓高 |

第二種容易出錯：戶號在「號」的位置。若只取「棟」前面那段，A棟1號到
A棟6號會全部併成同一欄——和築親親 244 筆就會擠進 4 欄，完全失去辨識度。

認不出來的寫法會整串當一欄，表頭保留原文，不會硬套規則。
