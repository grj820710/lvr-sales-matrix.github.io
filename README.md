...
| --- | --- | --- |
| 分季封存 | 已結束的季別 | 歷史資料的主體 |
| 發布日檔案 | 指定發布日（每月 1、11、21 日）那一批 | 分季檔只到上一季，當季要靠這個補；**解約情形也只能從這裡取得** |
| 當期 | 最近一批（約十天） | 最新成交 |

只用「分季 + 當期」會漏資料：當季還沒封存成季別檔，而當期只有最近十天，
中間發布的交易兩邊都碰不到。實測勝興豐川就因此少了一筆（115-05-24 那筆
落在 115S2 結束與當期起始之間的空窗）。

**解約資訊尤其依賴發布日檔案。** 解約是在原交易發布後才申報的，
已封存的季別檔不會回頭更新，所以只有申報日之後發布的批次才帶得到解約情形。
合併時來源一律「舊到新」處理，讓較新的快照覆蓋較舊的；順序顛倒會把解約洗掉。
...
執行時日誌會逐筆列出抓到的紀錄（戶別、樓層、日期、總價、棟及號、是否解約），
先跟網站上的清單逐列比對，看差的是哪一筆。

還看不出來，就到 Actions 手動觸發、把 **`dump`** 勾起來再跑一次。
這會額外印出：

- 來源主檔的完整表頭（確認欄名沒有被官方改過，程式的模糊比對還抓得到）
- 該縣市預售屋總列數，以及建案名稱相符的列數
- 每一列的 `棟及號`、`交易年月日`、`總價元`、`移轉層次`、`解約情形` 原始值

有了這些就能判斷差額是「開放資料本來就沒有那筆」還是「程式解析掉了」，
兩者的處理方式完全不同，不先分清楚會一直修錯地方。
